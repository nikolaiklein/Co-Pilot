...
class AnalyzerService:
    ...
    async def analyze_user_profile(self, user_id: int):
        ...
        try:
            ...
            # 3. Получаем текущий профиль пользователя, чтобы анализатор мог его дополнить
            ...
            prompt = f"""
Проанализируй диалог пользователя с ИИ-ассистентом.
Твоя задача: Обновить и дополнить "Досье Пользователя" (профиль), выделив ключевые аспекты для оптимизации его работы и жизни.

Ткущий профиль (если есть):
{current_profile}

Новый диалог для анализа:
{dialog_log}

Сформируй ОБНОВЛЕННЫЙ отчет в формате JSON.
Если в диалоге появилась новая информация — добавь её.
Если старая информация подтвердилась — оставь.
Если опроверглась — удали или исправи.
...
{{
  ...
  "dreams": ["мечта 1", "мечта 2", "идея 1"],
  "summary": "Краткое резюме текущего состояния и рекомендация (1-2 предложения)."
}}

Важно:
- Поле 'dreams' должно содержать мечты, желания и идеи пользователя.
- Не выдумывай факты. Если информации нет — оставляй поля как есть или пустыми.
- Ответ должен содержать только JSON код.
"""
